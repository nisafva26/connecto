...
final circlesProvider = StreamProvider.autoDispose<List<CircleModel>>((ref) {
  ...
  return userDocRef.snapshots().asyncMap((userDocSnapshot) async {
    final userData = userDocSnapshot.data();
    if (userData == null || !userData.containsKey('circles')) return [];

    final circleIds = List<String>.from(userData['circles'] ?? []);
    if (circleIds.isEmpty) return [];

    final circleDocs = await Future.wait(circleIds.map((circleId) async {
      final circleSnap = await FirebaseFirestore.instance
          .collection('circles')
          .doc(circleId)
          .get();
      ...
      return CircleModel.fromFirestore(circleSnap);
    }));
