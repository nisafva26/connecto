...
// helpers
async function batchDeleteQuery(q, size = 300) {
  while (true) {
...
async function chunked(iterable, n) {
  const out = [];
  let buf = [];
...
async function deleteSubcollection(colRefPath, size = 300) {
  const colRef = db.collection(colRefPath);
  while (true) {
...
exports.deleteAccount = onCall(async (req) => {
  ...
  // ---- 1) FRIENDSHIPS (your schema: userA, userB)
  await batchDeleteQuery(db.collection("friendships").where("userA", "==", uid));
  await batchDeleteQuery(db.collection("friendships").where("userB", "==", uid));

  // ---- 2) CHATS (your schema: participants [uidA, uidB], subcol messages with senderId/receiverId)
  ...
  for (const chat of chatsSnap.docs) {
    ...
    const updates = {
      participants: others.length ? others : [], // leave peer(s)
      peerDeleted: true,
      chatClosed: true,
      [`deletedBy.${uid}`]: true,
      peerDisplayName: "Deleted user",
      peerAvatarUrl: admin.firestore.FieldValue.delete(),
      // keep lastMessage as-is for history
    };

    const b = db.batch();
    b.update(chat.ref, updates);

    // system message so peer understands why chat is read-only
    const msgRef = chat.ref.collection("messages").doc();
    b.set(msgRef, {
      messageId: msgRef.id,
      isAutoGenerated: true,
      text: "This user deleted their account.",
      senderId: uid,                             // or "system"
      receiverId: others[0] ?? null,
      pingPattern: [],                           // keep your schema happy
  ...
  // 3) GATHERINGS  ------------------------------------------
  // 3a) If user is HOST → soft-cancel + mark deleted
  const hostSnap = await db.collection("gatherings").where("hostId", "==", uid).get();
  if (!hostSnap.empty) {
    const batches = await chunked(hostSnap.docs, 300);
    for (const docs of batches) {
      const b = db.batch();
      docs.forEach((d) =>
        b.update(d.ref, {
          status: "cancelled",                 // your app can respect this
          isDeleted: true,
          cancelledAt: admin.firestore.FieldValue.serverTimestamp(),
          cancelledReason: "host_deleted_account",
          hostId: admin.firestore.FieldValue.delete(),
        })
      );
      await b.commit();
    }
  ...
  // 3b) INVITEE cleanup: find gatherings where invitees.{uid} exists,
  // then remove both the map key and the subdoc invitees/{uid}
  try {
    const FieldPath = admin.firestore.FieldPath;
    const keyPath = new FieldPath("invitees", uid);

    // Any operator that checks existence works; '!=' null is common
    const gatheringsWithUser = await db
      .collection("gatherings")
      .where(keyPath, "!=", null)
      .get();

    const batchSize = 300;
    for (let i = 0; i < gatheringsWithUser.docs.length; i += batchSize) {
      const slice = gatheringsWithUser.docs.slice(i, i + batchSize);
      const b = db.batch();

      for (const g of slice) {
        // delete map entry
        b.update(g.ref, { [`invitees.${uid}`]: admin.firestore.FieldValue.delete() });

        // delete subcollection doc if present
        const inviteeDocRef = g.ref.collection("invitees").doc(uid);
        b.delete(inviteeDocRef);
      }

      await b.commit();
    }

    logger.info("gatherings_invitee_removed", { count: gatheringsWithUser.size });
  } catch (e) {
    logger.error("gatherings_invitee_error", e);
    // (optional) don't throw—let the rest of delete proceed
  }


  // ---- 4) USER DOC + SUBCOLLECTIONS under /users/{uid}
  const userRef = db.collection("users").doc(uid);
  // known subcollections from your screenshot
  for (const sc of ["chatFlags", "groupChatFlags"]) {
    await deleteSubcollection(`${userRef.path}/${sc}`);
  }
  ...
  // ---- 5) AUTH (do last; irreversible)
  await admin.auth().deleteUser(uid);

  logger.info(`Delete complete for uid=${uid}`);
  return { ok: true };
});
